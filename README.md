...
- `OnceDone-v1.0.5.dmg`：可拖拽安装的 macOS 安装包。
- `OnceDone-v1.0.5.sha256`：安装包 SHA-256 校验文件。

## v1.0.5 更新

- 降低空闲资源占用，JSON、OCR、翻译等重任务按需运行，关闭工具后更快释放内存。
- JSON 工具优化大文本编辑、详情栏释放和 `Esc` 失焦编辑器。
- 截图 OCR 与翻译交互更稳定，关闭 OCR 小窗口不会清空选区内识别结果。
- 设置、工具窗口和权限引导完成 UI/UX 治理，补齐图标按钮 hover 提示，优化加载、禁用、危险操作和确认状态。
- 权限引导支持在一个窗口内继续授权其他权限，并在窗口重新聚焦时刷新授权状态。
- 检查更新会展示下载、校验、安装和重启进度，安装重启前会再次确认。
- 存储页支持按工具清空 OnceDone 托管数据；新增重置应用，便于恢复到首次安装状态。
- 新增默认关闭的“锁定屏幕”工具，可选择锁定时同时关闭显示器。
...
1. 在 [Releases](https://github.com/bytebiu/OnceDone/releases) 下载 `OnceDone-v1.0.5.dmg`。
